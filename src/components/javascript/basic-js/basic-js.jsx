...
function BasicJS() {
  ...
  <script src="likes.js"></script>
      ...
      <p>Тег <code>&lt;script&gt;</code> подключается перед закрывающим тегом body - чтобы браузер сначала отобразил и распарсил (разобрал) страницу и только после этого перешел к выполнению скриптов.</p>
      ...
      <p>Так вот, если для загрузки сценариев применялся атрибут defer, то событие DOMContentLoaded сгенерируется только после окончания загрузки и выполнения всех таких сценариев.</p>
      ...
      <p className="hl-8">Атрибуты async и defer помогут оптимизировать загрузку и выполнение кода. Первый удобен для подключения счётчиков, аналитики и других подобных сценариев (когда внешний сценарий независим от выполнения других сценариев и не опирается на готовность DOM). Таким сценариям, как правило, не важна готовность DOM и последовательность выполнения. defer, наоборот, полезен там, где важна последовательность.</p>
...
/* И многострочные. Они могут отключить сразу несколько строк кода. */`}
      ...
      <p>Не стоит увлекаться комментированием. Лучше сфокусироваться на читабильности кода. Если подобрать «говорящие» имена для переменных и функций, то такой код вряд ли потребует дополнительных пояснений.</p>
      ...
      <p>Значением this (контекста) в нестрогом режиме при вызове функции был глобальный объект window. В строгом режиме значение this в таких случаях undefined.</p>
