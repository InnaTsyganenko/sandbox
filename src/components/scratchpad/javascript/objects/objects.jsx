...
console.log(cat.name); // Выведет в консоль: Рокки`}
      ...
      <pre>{`// Если в ES5 вам нужно было записать переменную
...
// { carModel: 'Volkswagen', name: 'Robert', phone: 'iPhone' }`}
      ...
      <h3>Заморозка объекта <span className="hl-1">Object.freeze()</span></h3>
      <p>Метод Object.freeze() «замораживает» объект, предотвращая изменение существующих свойств этого объекта или добавление новых свойств и значений в объект. Существует ошибочное мнение, что этот метод аналогичен объявлению объектов с использованием ключевого слова const, однако, объекты-константы модифицировать можно.</p>
      <p>«Заморозим» объект, после чего попытаемся изменить его свойство и убедимся в том, что сделать этого нельзя.</p>
      <pre>{`const frozenObject = {
  name: 'Robert'
}
...
// объект frozenObject не изменится, он сохранится в виде { name: 'Robert' }`}
      ...
      <h3>Запечатать объект <span className="hl-1">Object.seal()</span></h3>
      <p>Метод Object.seal() позволяет «запечатать» объект, предотвратив добавление новых свойств. При этом существующие свойства можно менять.</p>
      <p>«Запечатаем» объект, что не позволит добавить в него новое свойство, но оставит возможность менять существующие свойства.</p>
      <pre>{`const sealedObject = {
  name: 'Robert'
}
...
// объект sealedObject будет выглядеть так: { name: 'Bob' }`}
      ...
      <h3>Объединение объектов <span className="hl-1">Object.assign()</span></h3>
      <p>Метод Object.assign() позволяет объединять объекты, копируя свойства из одного объекта в другой. На самом деле, того же эффекта можно достичь с помощью вышеописанного оператора расширения, поэтому без этого метода вполне можно обойтись. Надо отметить, что этот метод, как и оператор расширения, не выполняет глубокого клонирования объектов. Если вам нужно готовое средство для глубокого клонирования объектов — взгляните на инструменты библиотеки Lodash.</p>
      <p>Создадим из двух объектов один.</p>
      <pre>{`const firstObject = {
  firstName: 'Robert'
}
