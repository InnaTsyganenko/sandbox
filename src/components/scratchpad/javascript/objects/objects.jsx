...
user; // {name: 'Harry Potter', age: 10}
Немного упростился и синтаксис объявления методов в объекте, теперь можно не писать ключевое слово function.
...
user.fire(); // 'Expecto Patronum'
В некоторых случаях, в объектах хочется использовать вычисляемые ключи, то есть создавать значения ключей с помощью квадратных скобок. Например, это может пригодиться при использовании объектов в качестве словарей. Новый синтаксис позволяет использовать вычисляемые ключи на этапе создания новых объектов, а не только при обращении к уже существующим.
...
console.log(cat.getGreeting()); // Выведет: Мяу, привет!`}
      ...
      <pre>{`
        ...
        wizard.level = -Infinity; // Уровня минус бесконечность не существует, поэтому будет взято значение 1 (-Infinity < 1 < 3)
        wizard.level; // Кажется, я Новичок
      `}
      </pre>
